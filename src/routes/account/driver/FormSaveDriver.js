...
class FormSaveDriver extends Component {
	static propTypes = {
	...
	async componentDidMount() {
		...
		let guide = this.props.listGroup.filter((item, index) => item.slug == 'driver');
		this.setState({
			...this.state,
	...
	static getDerivedStateFromProps(props, state) {
		if (props.account !== state.account) {
			let guide = props.listGroup.filter((item) => item.slug === 'driver');
			return {
				...state,
				groupid: props.edit
		...
		return (
			<React.Fragment>
				{open ? (
					<Modal
						title={
							edit ? <IntlMessages id="account.updateDriver" /> : <IntlMessages id="account.addDriver" />
						}
...
export default Form.create({ name: 'driver' })(connect(mapStateToProps, mapDispatchToProps)(FormSaveDriver));
