...
import FormSaveDriver from './FormSaveDriver';
const columns = [
	{
		key: 'image',
...
class ListDriver extends Component {
	...
	onAddAccount = () => {
		this.setState({ addDriverState: true });
	};
	onEditAccount = (account) => () => {
		this.setState({ addDriverState: true, current_account: account, edit: true });
	};
	onAccountClose = () => {
		this.setState({ addDriverState: false, current_account: null, edit: false });
	};
	...
	onRefresh = () => {
		...
		this.setState(
			...
			() => this.props.listAccountAction(this.state.filter, 'driver')
		);
	};
	getOrder = (order) => {
		...
		return (
			<React.Fragment>
				<div className="formelements-wrapper">
					<div className="row">
						<RctCollapsibleCard heading="List Driver" colClasses="col-12">
							<TableActionBar2
				...
				{listAccountReducer.isSuccess && (
					<FormSaveDriver
						open={this.state.addDriverState}
