...
import { Form, Input, Row, Col, Button, Modal, InputNumber } from 'antd';
...
class FormSaveRegistered extends Component {
	static propTypes = {
	...
	async componentDidMount() {
		...
		let guide = this.props.listGroup.filter((item, index) => item.slug == 'suppiler');
		this.setState({
			...this.state,
	...
	static getDerivedStateFromProps(props, state) {
		if (props.account !== state.account) {
			let guide = props.listGroup.filter((item) => item.slug === 'suppiler');
			return {
				...state,
				groupid: props.edit
					? props.account.groups.map((item) => item.id)
					: guide.length ? guide.map((item) => item.id) : [ null ],
		...
		this.props.form.validateFields((err, values) => {
			if (!err) {
				...
				if (this.props.account) {
					Object.assign(account, { id: this.props.account.id });
				}
			...
			<React.Fragment>
				{open ? (
					<Modal
						title={
							edit ? (
								<IntlMessages id="account.updateRegistered" />
							) : (
								<IntlMessages id="account.addRegistered" />
							)
						}
...
export default Form.create({ name: 'registered' })(connect(mapStateToProps, mapDispatchToProps)(FormSaveRegistered));
