...
class AddAdmin extends Component {
  ...
  async componentDidMount() {
    ...
    let admin = this.props.listGroup.filter(item => item.slug === 'admin')
    this.setState({
      ...this.state,
      groupid: this.props.edit ? this.props.account.groupid : admin.map(item => item.id),
  ...
  static getDerivedStateFromProps(props, state) {
    if (props.account !== state.account) {
      let admin = props.listGroup.filter(item => item.slug === 'admin')
      return {
        ...state,
        groupid: props.edit ? props.account.groups.map(item => item.id) : admin.length ? admin.map(item => item.id) : [null],
    ...
    return (
      <React.Fragment >
        {open ?
          <Modal
            title={edit ?
              <IntlMessages id="account.saveAdmin" /> :
              <IntlMessages id="account.addAdmin" />}
